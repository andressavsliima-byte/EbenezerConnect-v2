...
// Resolve imagens (Cloudinary URL completa ou legado /uploads/...)
const resolveImageSrc = (img) => {
  if (!img) return '/images/placeholder.svg';
  const s = String(img);
  if (s.startsWith('http')) return s;

  // Legado: /uploads/arquivo.jpg ou /dmruy8w87/image/upload/...
  const cloud = import.meta.env.VITE_CLOUDINARY_CLOUD_NAME;
  if (cloud && s.startsWith(`/${cloud}/image/upload`)) {
    return `https://res.cloudinary.com${s}`;
  }

  const apiBase = import.meta.env.VITE_API_URL; // .../api
  const origin = apiBase ? apiBase.replace(/\/api\/?$/, '') : '';
  return origin ? `${origin}${s.startsWith('/') ? s : `/${s}`}` : s;
};
...
export default function AdminProducts() {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      // ✅ Cloudinary retorna URL completa (https://res.cloudinary.com/...)
      const imageUrl = response.data?.url || response.data?.imageUrl || response.data?.secure_url;
